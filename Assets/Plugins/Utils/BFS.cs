...
public static class BFS<T> where T : BFSNode {

	public static IEnumerator GetAdjacentsIEnumerator(T[] nodes, Action<int[][]> onComplete) {
		int[][] adjacents = new int[nodes.Length][];
	...
	public static int[][] GetAdjacents(T[] nodes) {
		int[][] adjacents = new int[nodes.Length][];
		for (int i = 0; i < nodes.Length; i++) {
			nodes[i].Index = i;
		}
	...
	public static bool FindPath(T[] nodes, T startNode, T endNode, out List<T> path) {
		return FindPath(nodes, GetAdjacents(nodes), startNode, endNode, out path);
	}

	public static bool FindPath(T[] nodes, int[][] adjacents, T startNode, T endNode, out List<T> path) {
		path = new List<T>();
